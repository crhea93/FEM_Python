...
pseudo_time_max = 1 + 100 #second value is the number of steps
endTime = 100 #end of time
...
for i in range(len(NodalCoord)):
    x_inits[i] = NodalCoord[i][0]
    y_inits[i] = NodalCoord[i][1]

#Initial Mesh Plotted
...
while pt < (pseudo_time_max):
    ...
    for i in range(len(x_coords)):
        ...
        #count_coords += 1
    #Write to vtk
    vtkwrite("output/time_"+str(pt),len(NodalCoord),len(Connectivity),x_coords,y_coords,Connectivity)
