...
def vtkwritefield(filename,num_nodes_total,num_el,x_coords,mesh,field):
    ...
    for i in range(0,num_nodes_total):
        vtk.write(str(x_coords[i])+" "+"0.0"+" "+"0.0"+'\n')
    vtk.write('\n')
    vtk.write("POINT_DATA "+str(num_nodes_total)+'\n')
    vtk.write("SCALARS Radiance double"+'\n')
    vtk.write("LOOKUP_TABLE default"+'\n')
    for i in range(num_nodes_total):
        vtk.write(str(field[i][0])+'\n')
    ...
    vtk.write('\n')
    vtk.write("CELL_ TYPES "+str(num_el)+'\n')
    for i in range(0,num_el):
        vtk.write("9"+'\n')'''
    vtk.close()
